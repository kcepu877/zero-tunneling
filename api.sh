...
setup_bot() {
    print_header
    print_rainbow "🚀 Initializing API-TUNNELING Setup..."
    ...
    # === Extract API Files ===
    if [ ! -f /usr/bin/API-TUNNELING/api.js ]; then
        echo -e "${blue}📁 Downloading API-TUNNELING package...${neutral}"
        curl -sL "https://raw.githubusercontent.com/kcepu877/zero-tunneling/main/api/API-TUNNELING.zip" -o /usr/bin/API-TUNNELING.zip
        cd /usr/bin && 7z x -punlock API-TUNNELING.zip >/dev/null 2>&1
        rm API-TUNNELING.zip* && chmod +x API-TUNNELING/* && cd
    fi

    # === Install Dependencies ===
    npm list --prefix /usr/bin/API-TUNNELING express child_process >/dev/null 2>&1 || {
        echo -e "${yellow}📦 Installing dependencies...${neutral}"
        npm install --prefix /usr/bin/API-TUNNELING express child_process
    }
    ...
    MESSAGE="🚀 *API-TUNNELING Installed Successfully* 🚀
🔑 *Auth Key:* \`$AUTH_KEY\`
🌐 *Server IP:* \`$SERVER_IP\`
🌍 *Domain:* \`$DOMAIN\`"
